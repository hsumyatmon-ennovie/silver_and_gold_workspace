CREATE OR ALTER VIEW [prod].[gold_shipment_plan_status]
AS
/* =========================
...
/* 3) Routing: เลือก CELL ตัวแรกต่อ (prod_order_no, prod_order_lineno) โดยดัน CELL108 ไปท้าย  (คงเดิม) */
RoutingRanked AS (
    SELECT
        rl.prod_order_no,
        rl.item_no,
...
LatestStatus AS (
    SELECT
        ...
        ROW_NUMBER() OVER (
            PARTITION BY s.prod_order_no, TRY_CONVERT(int, s.prod_order_line_no), s.current_location_code
...
Production AS (
    ...
    WHERE (pl.prod_order_status IS NULL OR pl.prod_order_status IN ('Released'))
...
SELECT DISTINCT
    ...
    CASE
        WHEN p.casting_status IS NOT NULL               THEN p.casting_status
        WHEN p.current_location_code_latest IS NOT NULL THEN p.current_location_code_latest
        WHEN p.itemFG_Category = 'CASTING'              THEN 'WAX'
        ELSE 'RELEASED'
...
-- ORDER BY s.ShipmentWeek DESC,p.prod_order_starting_date_time DESC
-- OFFSET 0 ROWS FETCH NEXT 1000 ROWS ONLY;
 
 
    -- /* ---- Sales ---- */
    -- s.SalesorderNo, s.CusNo, s.CusName, s.CusAbbr,
    -- s.StatusSO, s.ReqDate, s.PmDate, s.CSNoted, s.CSteam,
    -- s.sales_order_external_document, s.sales_order_released_date, s.ShipmentWeek,
    -- s.SalesLineNo,
    -- s.ItemFG, s.item_description, s.Total_QTY, s.LineShipmentDate, s.TypeofFG, s.OutstandingQty,
 
    -- /* ---- Production ---- */
    -- p.prod_order_no,
    -- p.sales_order_line_no                         AS prod_order_line_no,
    -- p.ref_item                                    AS prod_item_no,
    -- p.item_routing_no,
    -- p.prod_order_due_date                         AS prod_order_due_date,
    -- p.prod_order_starting_date_time               AS prod_line_start_date,
    -- p.prod_order_ending_date_time                 AS prod_line_end_date,
    -- p.prod_order_quantity                         AS prod_line_quantity,
    -- NULL                                          AS prod_line_finished_quantity,
    -- NULL                                          AS prod_line_remaining_quantity,
    ...
    -- /* ---- Window calcs ---- */
    -- MAX(CASE WHEN p.sales_order_line_no = 10000 THEN p.prod_order_starting_date_time END)
    --     OVER (PARTITION BY p.prod_order_no)                                          AS FG_duedate,
    -- MAX(CASE WHEN p.item_routing_no = 'CST_CUT' THEN p.prod_order_due_date END)
    --     OVER (PARTITION BY p.prod_order_no, p.ref_item)                              AS Casting_duedate,
 
    -- /* ---- Casting (simple join) ---- */
    -- p.itemCST,
    -- p.casting_prod_order,
    -- p.casting_tree_no,
    -- p.casting_qty_to_tree,
    -- p.casting_qty_passed,
    -- p.casting_qty_reject,
 
    -- /* ---- Latest status ---- */
    -- p.status_created_on_utc,
    -- p.current_location_code_latest                 AS current_location_code,
    -- p.casting_status,
    -- CASE
    --     WHEN p.casting_status IS NOT NULL               THEN p.casting_status
    --     WHEN p.current_location_code_latest IS NOT NULL THEN p.current_location_code_latest
    ...
    -- /* ---- Routing summary ---- */
    -- p.first_cell_no                 AS first_cell_cellno,
    -- p.prod_line                     AS Prod_line_no,
    -- p.routing_item_no               AS routing_item_no,
    -- p.routing_line_no               AS routing_line_no,
 
    -- /* ---- Due & Risk ---- */
    -- DATEDIFF(DAY, CAST(GETDATE() AS date), p.prod_order_due_date) AS days_to_due,
    -- CASE
    --     WHEN p.prod_order_due_date IS NULL THEN NULL
    --     WHEN DATEDIFF(DAY, CAST(GETDATE() AS date), p.prod_order_due_date) < 0
    --          THEN CONCAT('Overdue ', ABS(DATEDIFF(DAY, CAST(GETDATE() AS date), p.prod_order_due_date)), 'd')
