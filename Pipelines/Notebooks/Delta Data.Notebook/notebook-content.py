...
DEDUPE_SPECS = [
    ...
    },
    {
        ...
        "keys":   ["Work_Day", "Employee_Code", "actual_date_time_in"],
...
# MAGIC %%sql
# MAGIC CREATE OR REPLACE TABLE Gold_Production_Lakehouse.delta.gold_delta_attendance
# MAGIC USING DELTA
# MAGIC AS
# MAGIC 
# MAGIC WITH emp_master AS (
# MAGIC     SELECT
# MAGIC         employee_code,
# MAGIC         TRIM(REGEXP_REPLACE(department, ' +', ' ')) AS department
# MAGIC     FROM Gold_Production_Lakehouse.prod.gold_employee_data_daily
# MAGIC     WHERE end_date IS NULL
# MAGIC       AND TRIM(REGEXP_REPLACE(department, ' +', ' ')) LIKE 'PROD LINE %'
# MAGIC ),
# MAGIC 
# MAGIC emp_daily AS (
# MAGIC     SELECT
# MAGIC         t.Work_Day AS work_day,
# MAGIC         TRIM(REGEXP_REPLACE(t.Department_Eng, ' +', ' ')) AS department,
# MAGIC         COUNT(DISTINCT t.Employee_Code) AS present_count
# MAGIC     FROM Gold_Commons_Lakehouse.cmn.gold_employee_time_all t
# MAGIC     INNER JOIN emp_master m
# MAGIC         ON t.Employee_Code = m.employee_code
# MAGIC        AND TRIM(REGEXP_REPLACE(t.Department_Eng, ' +', ' ')) = m.department
# MAGIC     WHERE t.Work_Day >= date_sub(current_date(), 30)
# MAGIC       AND t.Work_Day <= current_date()
# MAGIC       AND TRIM(REGEXP_REPLACE(t.Department_Eng, ' +', ' ')) LIKE 'PROD LINE %'
# MAGIC     GROUP BY
# MAGIC         t.Work_Day,
# MAGIC         TRIM(REGEXP_REPLACE(t.Department_Eng, ' +', ' '))
# MAGIC ),
# MAGIC 
# MAGIC emp_total AS (
# MAGIC     SELECT
...
# MAGIC         d.work_day,
# MAGIC         d.department,
# MAGIC         LEAST(d.present_count, e.total_employees) AS present_count,
# MAGIC         e.total_employees,
# MAGIC         CASE
# MAGIC             WHEN e.total_employees > 0
# MAGIC                 THEN LEAST(d.present_count, e.total_employees) / e.total_employees
# MAGIC             ELSE 0
# MAGIC         END AS attendance_rate
# MAGIC     FROM emp_daily d
# MAGIC     INNER JOIN emp_total e
# MAGIC         ON d.department = e.department
# MAGIC )
# MAGIC 
# MAGIC SELECT
# MAGIC     work_day,
...
# MAGIC     attendance_rate,
# MAGIC     AVG(attendance_rate) OVER (PARTITION BY department) AS avg_attendance_rate_30d
# MAGIC FROM attendance;
...
# MAGIC     WHEN upper(ml.loss_after_scrap_std_flag) IN ('Missing','ABOVE','HIGH') THEN 'OVER_STANDARD'
# MAGIC     WHEN upper(ml.loss_after_scrap_std_flag) IN ('OK','BELOW','LOW') THEN 'UNDER_STANDARD'
...
# MAGIC %%sql
# MAGIC CREATE OR REPLACE TABLE Gold_Production_Lakehouse.delta.gold_delta_production_detail_runtime AS
# MAGIC WITH routing_line AS (
# MAGIC     SELECT
# MAGIC         `Routing No.`           AS routing_no,
# MAGIC         `Routing Reference No.` AS routing_reference_no,
# MAGIC         `No.`                   AS routing_step,
# MAGIC         `Run Time`              AS run_time,
# MAGIC         `Status`                AS routing_status,
# MAGIC         `Prod. Order No.`       AS prod_order_no,
# MAGIC         `Input Quantity`        AS input_quantity,
# MAGIC         -- (`Input Quantity` * `Run Time`) AS total_runtime,
# MAGIC         `Starting Date-Time`    AS routing_start_datetime,
# MAGIC         `Ending Date-Time`      AS routing_end_datetime,
# MAGIC         `Location Code`         AS location_code
# MAGIC     FROM Silver_BC_Lakehouse.bc.`Prod Order Routing Line`
# MAGIC     
# MAGIC ),
# MAGIC production_detail AS (
# MAGIC     SELECT *
# MAGIC     FROM Gold_Production_Lakehouse.delta.gold_delta_production_detail
# MAGIC     
# MAGIC )
# MAGIC SELECT
# MAGIC     p.*,
# MAGIC     r.routing_no,
# MAGIC     r.routing_step,
# MAGIC     r.run_time,
# MAGIC     r.routing_status,
# MAGIC     r.input_quantity,
# MAGIC     (r.run_time * p.prod_line_quantity) AS total_runtime,
# MAGIC     r.routing_start_datetime,
# MAGIC     r.routing_end_datetime,
# MAGIC     r.location_code
# MAGIC FROM production_detail p
# MAGIC LEFT JOIN routing_line r
# MAGIC     ON p.prod_order_no = r.prod_order_no
# MAGIC    AND p.prod_order_line_no = r.routing_reference_no
# MAGIC    AND p.current_stage_location = r.routing_step;
...
# MAGIC 
# MAGIC     -- Yesterday
# MAGIC     COUNT(CASE WHEN TO_DATE(Date) = DATE_SUB(CURRENT_DATE(), 1) AND loss_after_scrap_std_flag = 'OK' THEN 1 END) AS yesterday_under,
# MAGIC     COUNT(CASE WHEN TO_DATE(Date) = DATE_SUB(CURRENT_DATE(), 1) AND loss_after_scrap_std_flag = 'Missing'  THEN 1 END) AS yesterday_over,
# MAGIC 
# MAGIC     -- This Week (Monday → today)
# MAGIC     COUNT(CASE WHEN TO_DATE(Date) >= DATE_SUB(CURRENT_DATE(), DAYOFWEEK(CURRENT_DATE()) - 2) AND loss_after_scrap_std_flag = 'OK' THEN 1 END) AS this_week_under,
# MAGIC     COUNT(CASE WHEN TO_DATE(Date) >= DATE_SUB(CURRENT_DATE(), DAYOFWEEK(CURRENT_DATE()) - 2) AND loss_after_scrap_std_flag = 'Missing'  THEN 1 END) AS this_week_over,
# MAGIC 
...
# MAGIC                AND loss_after_scrap_std_flag = 'OK' THEN 1 END) AS last_week_under,
# MAGIC     COUNT(CASE WHEN TO_DATE(Date) >= DATE_SUB(CURRENT_DATE(), DAYOFWEEK(CURRENT_DATE()) + 5)
# MAGIC                AND TO_DATE(Date) <  DATE_SUB(CURRENT_DATE(), DAYOFWEEK(CURRENT_DATE()) - 2)
# MAGIC                AND loss_after_scrap_std_flag = 'Missing'  THEN 1 END) AS last_week_over,
# MAGIC 
# MAGIC     -- All Time
# MAGIC     COUNT(CASE WHEN loss_after_scrap_std_flag = 'OK' THEN 1 END) AS alltime_under,
# MAGIC     COUNT(CASE WHEN loss_after_scrap_std_flag = 'Missing'  THEN 1 END) AS alltime_over
# MAGIC 
...
# MAGIC   WHERE ml_material_type != 'Brass'
# MAGIC   GROUP BY
# MAGIC     ml_prod_line,
# MAGIC     ml_cell_line,
# MAGIC     ml_employee_no,                                       -- ✅ ADD THIS
...
# MAGIC 
# MAGIC     -- Counts
# MAGIC     COUNT(CASE WHEN loss_std_flag = 'OK' THEN 1 END)                AS under_count,
# MAGIC     COUNT(CASE WHEN loss_std_flag = 'Missing'  THEN 1 END)                AS over_count,
# MAGIC 
# MAGIC     -- Total loss_after_dust_g
# MAGIC     SUM(CASE WHEN loss_std_flag = 'OK' THEN loss_g END)  AS under_loss_g,
# MAGIC     SUM(CASE WHEN loss_std_flag = 'Missing'  THEN loss_g END)  AS over_loss_g,
# MAGIC 
# MAGIC     -- Average loss_after_dust_g
# MAGIC     ROUND(AVG(CASE WHEN loss_std_flag = 'OK' THEN loss_g END), 4) AS under_avg_loss_g,
# MAGIC     ROUND(AVG(CASE WHEN loss_std_flag = 'Missing'  THEN loss_g END), 4) AS over_avg_loss_g
# MAGIC 
...
# MAGIC     `Operation Group` as operation_group,
# MAGIC     `Operation Sequence` as operation_sequence,
# MAGIC     `Department Group` as department
# MAGIC FROM Silver_BC_Lakehouse.bc.`Machine Center`
# MAGIC WHERE Blocked = 0
# MAGIC ORDER BY `Operation Sequence`
...
# MAGIC %%sql
# MAGIC CREATE OR REPLACE TABLE Gold_Production_Lakehouse.delta.gold_delta_cell_positions
# MAGIC SELECT
# MAGIC     sub_department_Eng AS cell,
# MAGIC     concat_ws(', ', sort_array(collect_set(Position_Eng))) AS positions
# MAGIC FROM Gold_Production_Lakehouse.prod.gold_employee_time
# MAGIC WHERE sub_department_Eng LIKE 'CELL%'
# MAGIC   AND Work_Day = CURRENT_DATE()
# MAGIC   AND UPPER(TRIM(Position_Eng)) NOT IN (
# MAGIC         'RUNNER',
# MAGIC         'ASSISTANT TECHNICAL MANAGER',
# MAGIC         'ASSISTANT PRODUCT DEVELOPMENT MANAGER',
# MAGIC         '-',
# MAGIC         'TECHNICAL DIRECTOR',
# MAGIC         -- 'GENNERAL',
# MAGIC         -- 'GERNERAL',
# MAGIC         -- 'GENERAL',
# MAGIC         'ADMIN',
# MAGIC         'ENGINEER OFFICER ',
# MAGIC         'ENGINEER OFFICER'
# MAGIC   )
# MAGIC GROUP BY
# MAGIC     sub_department_Eng
# MAGIC ORDER BY
# MAGIC     sub_department_Eng;
...
# MAGIC %%sql
# MAGIC CREATE OR REPLACE TABLE Gold_Production_Lakehouse.delta.gold_delta_cell_positions_count AS
# MAGIC SELECT
# MAGIC     department AS prod_line,
# MAGIC     sub_department AS cell,
# MAGIC     concat_ws(
# MAGIC         ', ',
# MAGIC         sort_array(
# MAGIC             collect_list(concat(position, ' (', cnt, ')'))
# MAGIC         )
# MAGIC     ) AS positions
# MAGIC FROM (
# MAGIC     SELECT
# MAGIC         department,
# MAGIC         sub_department,
# MAGIC         TRIM(position) AS position,
# MAGIC         COUNT(*) AS cnt
# MAGIC     FROM Gold_Production_Lakehouse.prod.gold_employee_data_daily
# MAGIC     WHERE sub_department LIKE 'CELL%'
# MAGIC       AND end_date IS NULL
# MAGIC       AND UPPER(TRIM(position)) NOT IN (
# MAGIC             'RUNNER',
# MAGIC             'ASSISTANT TECHNICAL MANAGER',
# MAGIC             'ASSISTANT PRODUCT DEVELOPMENT MANAGER',
# MAGIC             '-',
